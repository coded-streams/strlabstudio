...
// ── Override renderJobList from perf.js to enforce session-scoped job visibility ─
// perf.js calls renderJobList(jobs) with ALL cluster jobs.
// We intercept here to filter for normal sessions — admin sees everything.
(function() {
  function _installJobListFilter() {
    if (typeof renderJobList !== 'function') return false;
    const _origRenderJobList = renderJobList;
    window.renderJobList = function(jobs) {
      // Filter based on session type — admin sees all, regular sees own only
      const filtered = (typeof filterJobsForCurrentSession === 'function' && typeof state !== 'undefined')
          ? filterJobsForCurrentSession(jobs)
          : jobs;
      _origRenderJobList(filtered);
      // Also update the perf jobs badge to reflect filtered count
      const badge = document.getElementById('ps-jobs-badge');
      if (badge) {
        const runCount = (filtered || []).filter(j => j.state === 'RUNNING').length;
        badge.textContent = runCount > 0
            ? runCount + ' running'
            : (filtered || []).length + ' total';
      }
      // If no jobs for this session, show a contextual empty message
      if (!state.isAdminSession && jobs.length > 0 && filtered.length === 0) {
        const list = document.getElementById('perf-job-list');
        if (list) list.innerHTML = `<div style="font-size:11px;color:var(--text3);padding:8px 0;line-height:1.7;">
          No jobs submitted in this session.<br>
          <span style="font-size:10px;">${jobs.length} job(s) running on the cluster belong to other sessions. Connect as Admin to see all cluster jobs.</span>
        </div>`;
      }
    };
    return true;
  }
  if (!_installJobListFilter()) {
